  ...
  async execute(interaction){
    ...
    if(sub === 'add'){
      ...
      if(entries.find(e=>e.trigger.toLowerCase() === trigger.toLowerCase())){
        return interaction.reply({ content: 'Trigger already exists.', ephemeral: true });
      }
      ...
      return interaction.reply({ content: 'Auto-response added.', ephemeral: true });
    }
    if(sub === 'list'){
      if(!entries.length) return interaction.reply({ content: 'No auto-responses.', ephemeral: true });
    ...
    if(sub === 'remove'){
      ...
      if(entries.length === before) return interaction.reply({ content: 'Trigger not found.', ephemeral: true });
      await writeJSON('autoresponses.json', entries);
      await log('auto.remove', { userId: interaction.user.id, trigger });
      return interaction.reply({ content: 'Auto-response removed.', ephemeral: true });
    }
  }
};
